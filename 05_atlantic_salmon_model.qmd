...
#how far apart in the sequence are the farms? If the previouys is incomplete we should be able to use the one before
...
farms_to_omit <- mean_farm_temp |> filter(mean_temp<7) |> pull(farm_id)
...
farm_list <- 
  ...
  filter(model_name == this_species & !farm_id %in% farms_to_omit) |> 
  group_by(farm_id) |> 
  group_split()
    

#test 
#this_farm <- farm_list[[40]]
