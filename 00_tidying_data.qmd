...
Distribute feed summaries into each species folder

```{r}

feed_summaries <- 
  read.csv("data/_general_data/diets/protein_lipid_carb_summaries.csv") |> 
  select(-X) |> 
  pivot_longer(values_to = "prop", names_to = "nutrient", cols = -c(species)) |> 
  relocate(prop, .before = nutrient) |> 
  mutate(nutrient = paste0(nutrient, "s"))
  group_by(species) |> 
  group_split()
