...
base_rast <- rast(crs = "EPSG:4326", ext = c(-180,180, -90, 90), res = 0.5)
values(base_rast) <- 1:ncell(base_rast)

cell_id <- terra::as.data.frame(base_rast, xy=TRUE) |> rename(cell_id = lyr.1, CenterLong = x ,  CenterLat = y)


aquamaps_0.5d <- data.table::fread(file.path(rdsi_data_dir, "aquamaps/aquamaps_0.6_depth_prepped.csv")) |> left_join(cell_id)
...
#save to project
qsave(x = aquamaps_0.5d, file = here("data/_general_data/species_layers/distribution/aquamaps_0.5d_w_id.qs"))
