...
RK_solver <- \(Param, Temp, Feeding, Feed, IC, times){
  ...
  for (t in ti:(tf - 1)) {
    ...
    output <- \(Param, Temp, Feed, Gapp){
      ...
      betacarb <-  Param[5]# assimilation coefficient for carbohydrate
      epsprot <-  Param[6]# energy content of protein (J/g protein)
      epslip <-  Param[7]#energy content of lipid (J/g lipid)
      epscarb <-  Param[8]# energy content of carbohydrates (J/g carbohydrate)
      epsO2<-  Param[9] # energy consumed by respiration of 1 g of O2
      pk <-  Param[10]# temperature coefficient for fasting catabolism
      k0 <-  Param[11]# fasting catabolism at 0oC
      m <-  Param[12]# weight exponent for the anabolism
      n <-  Param[13]# weight exponent for the catabolism
      betac <-  Param[14]# shape coefficient for H(Tw) function
      Tma <-  Param[15]#maximum lethal temperature for Sparus aurata
      Toa <-  Param[16]#optimal temperature for Sparus aurata
      Taa <- Param[17] #Lowest feeding temperature for sparus aurata
      omega <-  Param[18] #oxygen consumption weight loss ratio
      a  <-  Param[19]#energy content of fish
      ...
      Pcont=Feed[1]       # Percentage of proteins in the food
      Lcont=Feed[2]       # Percentage of lipids in the food
      Ccont=Feed[3]       #  Percentage of carbohydrates in the food
      
      
      #Temperature
      Temp <- Temp # water temperature oC
      
      #Feeding 
      Gapp <- Gapp # g of food per day
      
      
      #H(Tw) function
      temperaturefun = (((Tma-Temp)/(Tma-Toa))^betac*Tma) * exp(betac*(Temp-Toa))
      
      # K(Tw) function 
      KTw = exp(pk*Temp)
      
      #weight 
      weight = if_else(i = ti, TRUE = outputs[[5]], FALSE = weights[i])
      
      
      # EXPRESSIONS FOR NET ANABOLISM
      
      ing = ingmax*temperaturefun*(weight^m) # Potential ingestion rate with maximum ingrestion scaled 
      
      #food limitation
      if(ing >= G){
        ing = G # 
      } else {
        ing = ing # otherwise use actual food ingrestion rate
      }
      ...
      A = (1-alpha)*ing*(Pcont*epsprot*betaprot + Ccont*epscarb*betacarb + Lcont*epslip*betalip) #Anabolism
      Fcs = ing*(Pcont*(1-betaprot) + Ccont*(1-betacarb) + Lcont*(1-betalip))
      ...
      return(dw, excretion, ingestion, ingestionvero, temperaturefun, metabolism, oxygen, ammonia)
...
ind_equations(Param = outputs[[1]], Temp = outputs[[2]], Feed = outputs[[4]], Feeding = output[[3]])
