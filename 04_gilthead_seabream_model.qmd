...
#create folders for parameters and forcings
filepaths <- c(sprintf(here("data/forcings/%s_forcings"), species), sprintf(here("data/parameters/%s_parameters"), species) )

for (path in filepaths){if(!file.exists(path)){dir.create(path)}}
...
ind_equations <- \(Param, Temp, Feed, Feed_avail){
  ...
  #Feeding 
  Feed_avail <- Feed_avail # g of food per day
  
  
  #H(Tw) function
  
  HTw = (((Tma-Temp)/(Tma-Toa))^betac*Tma) * exp(betac*(Temp-Toa))
